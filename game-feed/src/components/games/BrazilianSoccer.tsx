...
const BALL_SPEED = 2
const KICK_POWER = 5
const GAME_DURATION = 90 // 1.5 minutes in seconds
...
interface BrazilianSoccerProps {
  onScoreUpdate?: (score: number) => void;
}

export default function BrazilianSoccer({ onScoreUpdate }: BrazilianSoccerProps) {
  // Game canvas
  ...
  const checkGoals = () => {
    ...
    if (ball.x + ball.width > rightGoal.x && ball.y > rightGoal.y && ball.y < rightGoal.y + rightGoal.height) {
      playSound("goal")
      setPlayerScore((prev) => {
        const newPlayerScore = prev + 1;
        // Update total score (for parent component)
        setTotalScore(prevTotal => prevTotal + 100); // 100 points per goal
  ...
  return (
    <div className="relative w-full h-full flex items-center justify-center bg-black">
      ...
      <div className="absolute bottom-4 left-0 right-0 text-center text-white text-sm">
        {gameState === "playing" ? "SPACE/TAP: KICK" : "TAP TO START"}
      </div>
    </div>
  )
}